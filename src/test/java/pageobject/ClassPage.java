...
public class ClassPage  extends Baseclass {
	...
	public  classdetailpage(class_manage_obj,String comments) {
		act.Sendkeys(driver, "comments");
		act.Click(driver, click);
		
		act.SelectByIndex( dropdownBatchId,"ID");
		act.SelectByIndex( Noofclasses,"class");
		act.SelectByIndex( ClassDate,"date");
		act.SelectByIndex( Classtopic,"topic");
		act.SelectByIndex( StaffId,"Id");
		classmanageObj=new class_manage_Obj();
		return classmanageObj;
 ...
 public boolean correctSpelling()
	 ...
	 { popupactual.add(dropdownBatchId.getText());
	  popupactual.add(Noofclasses.getText());
	  popupactual.add(ClassDate.getText());
	 popupactual.add(Classtopic.getText()); 
	 popupactual.add(StaffId.getText());
	  popupexpected.add("dropdownBatchId");
	  popupexpected.add("Noofclasses");
	  popupexpected.add("ClassDate"); 
	  popupexpected.add("Classtopic");
	  popupexpected.add("StaffId"); 
	...
	public void dropdownBatchID()
		{

		Frame f= new Frame();
		Choice c=new Choice();
		c.setBounds(100,100, 75,75);
		c.add("Item 1 by 8586");
		c.add("Item 2 by 7172");
		c.add("Item 3 by 7654");
		f.add(c);
		f.setSize(400,400);
		f.setLayout(null);
		f.setVisible(true);
	 ...
	 */
	public void Topic() {
		Classtopic.sendKeys("Selenium");
	}

	public void StaffID() {
		StaffId.sendKeys("U02");
	}

	public void ClassDescrption() {
		Classdescrption.sendKeys("This is Automation Testing Class");
	}

	public void comments() {
		comments.sendKeys("Course Complected");
	}

	public void Notes() {
		Notes.sendKeys("SQL class pending");
	}

	public void Records() {
		Records.sendKeys("Recording session available");
	}
	...
	public void checkleftarrow() {
		try {
			...
			if (isLeftArrowDisabled) {
				Logger.info("Left arrow is disabled on page one when entries are more than 5 available in Class");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void pagination_disable() {
		if (Classcount < 5) {
			...
			if (!isLefttArrowEnabled) {
				Logger.info("Left arrow is disabled");
			}

			if (!isRightArrowEnabled) {
				Logger.info("Right arrow is disabled");
			}
